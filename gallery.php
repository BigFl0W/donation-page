...
$gallerySettings = Database::fetchAll("SELECT setting_key, setting_value FROM settings WHERE setting_key LIKE 'gallery_%'") ?: [];
$galleryPageSettings = [];
foreach ($gallerySettings as $gallerySetting) {
    $galleryPageSettings[$gallerySetting['setting_key']] = $gallerySetting['setting_value'];
}
...
$galleryFeedItems = $featuredGalleryItem ? array_slice($galleryItems, 1) : $galleryItems;
$page_title = "Photo & Video Gallery";
$breadcrumb_title = "Photo & Video Gallery";
$hero_title = trim((string)($galleryPageSettings['gallery_hero_title'] ?? 'Photo & Video Gallery'));
$hero_bg_image = trim((string)Helpers::setting('inner_page_banner_image', 'assets/images/breadcrumbs_bg.jpg'));
...
<section class="wide-tb-100 bg-light-gray">
    <div class="container">
        ...
        <?php if ($galleryFeedItems): ?>
        <div class="row g-4">
                <?php foreach ($galleryFeedItems as $item): ?>
